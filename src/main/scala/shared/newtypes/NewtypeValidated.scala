...
/** A validated [[Newtype]].
  ...
  *
  * Example: {{{ type EmailAddress = EmailAddress.Type
  *
  * object EmailAddress extends NewtypeValidated[String, Exception] { def
  * apply(v: String): Either[Exception, EmailAddress] = if (v.contains("@"))
  * Right(unsafeBuild(v)) else Left(new IllegalArgumentException("Not a valid
  * email")) } }}}
  */
